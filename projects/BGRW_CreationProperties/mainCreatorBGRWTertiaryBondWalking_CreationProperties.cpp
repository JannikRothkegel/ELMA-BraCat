...
#include "UpdaterCreatorBGRWTertiaryBondWalking_CreationProperties.h"
  ...
  try{
	...
	while ((option_char = getopt (argc, argv, "f:n:p:t:b:s:h"))  != EOF){
		...
		case 'b':
			...
			break;        
		case 'h': std::cout << "This program creates a branched structure on an evolving creation process using BGRW." << std::endl;
    ...
    UpdaterCreatorBGRWTertiaryBondWalking_CreationProperties<Ing> U_CW_CreationProperties(myIngredients, number_of_monomers, probability, probabilityTertiaryWalk, box, box, box);

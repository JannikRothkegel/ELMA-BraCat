...
#include "UpdaterCreatorHyperdendrimer.h"
...
int main(int argc, char* argv[])
{
  try{
	...
	std::string filename="Hyperdendrimer.bfm";
	...
	while ((option_char = getopt (argc, argv, "f:g:b:s:h"))  != EOF){
		...
		case 'b':
			...
			break;        
		case 'h': std::cout << "This program creates a hyperdendrimer branched structure." << std::endl;
    ...
    UpdaterCreatorHyperdendrimer<Ing> U_CW(myIngredients, maxfunctionality, box, box, box);

...
#ifndef LEMONADE_UPDATER_CREATOR_HYPERDENDRIMER_H
#define LEMONADE_UPDATER_CREATOR_HYPERDENDRIMER_H
/**
 * @file
 *
...
class UpdaterCreatorHyperdendrimer: public UpdaterAbstractCreate<IngredientsType>
{
	typedef UpdaterAbstractCreate<IngredientsType> BaseClass;

public:
	UpdaterCreatorHyperdendrimer(IngredientsType& ingredients_, uint32_t _maxfunctionality, uint32_t _box_x=128, uint32_t _box_y=128, uint32_t _box_z=128);
...
private:
	...
	//! Maximum Functionality (=generation) of hyperstar
	uint32_t maxfunctionality;
 ...
 */
template < class IngredientsType >
UpdaterCreatorHyperdendrimer<IngredientsType>::UpdaterCreatorHyperdendrimer(IngredientsType& ingredients_, uint32_t _maxfunctionality, uint32_t _box_x, uint32_t _box_y, uint32_t _box_z):
BaseClass(ingredients_), maxfunctionality(_maxfunctionality), boxX(_box_x), boxY(_box_y), boxZ(_box_z)
{
 ...
 */
template < class IngredientsType >
void UpdaterCreatorHyperdendrimer<IngredientsType>::initialize(){
 ...
 */
template < class IngredientsType >
bool UpdaterCreatorHyperdendrimer<IngredientsType>::execute(){
	std::cout << "executeUpdaterCreatorHyperdendrimer" << std::endl;
	...
	std::queue <uint32_t> queueLeaves; // Queue for algorithm holding leaves
	
	//queueLeaves.push(0);
        
        //while ((queueLeaves.size() > 0))
        {
            //uint32_t idx = queueLeaves.front();
            //queueLeaves.pop();
            
            //create neighbors
            uint32_t maxfunc = MaxNodeFunctionality[0];
            
            for(uint32_t k = 1; k <= maxfunc; k++)
            {
              addMonomerToParentWithProbability(0, (ingredients.getMolecules()[0].getAttributeTag()%2)+1);
              uint32_t addIdx = ingredients.getMolecules().size()-1;
              
              std::cout << 0 << " --> " << addIdx << " .f: " << (maxfunc-1) << std::endl;
              
              MaxNodeFunctionality[addIdx] = maxfunc-1;
               
              queueLeaves.push(addIdx);
            }
	...
	while ((queueLeaves.size() > 0))
        {
            ...
            //create neighbors
            uint32_t maxfunc = MaxNodeFunctionality[idx];
            
            for(uint32_t k = 1; k < maxfunc; k++)
            {
              addMonomerToParentWithProbability(idx, (ingredients.getMolecules()[idx].getAttributeTag()%2)+1);
              uint32_t addIdx = ingredients.getMolecules().size()-1;
              
              std::cout << idx << " --> " << addIdx << " .f: " << (maxfunc-1) << std::endl;
...
template<class IngredientsType>
bool UpdaterCreatorHyperdendrimer<IngredientsType>::addMonomerToParentWithProbability(uint32_t parent_id, int32_t type){
 ...
 */
template < class IngredientsType >
void UpdaterCreatorHyperdendrimer<IngredientsType>::cleanup(){
    ...
    commentHG_FunctionalityNodes << " File produced by analyzer UpdaterCreatorHyperdendrimer" << std::endl

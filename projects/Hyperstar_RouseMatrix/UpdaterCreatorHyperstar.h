...
#ifndef LEMONADE_UPDATER_CREATOR_HYPERSTAR_H
#define LEMONADE_UPDATER_CREATOR_HYPERSTAR_H
/**
 * @file
 *
...
class UpdaterCreatorHyperstar: public UpdaterAbstractCreate<IngredientsType>
{
	typedef UpdaterAbstractCreate<IngredientsType> BaseClass;

public:
	UpdaterCreatorHyperstar(IngredientsType& ingredients_, uint32_t _maxfunctionality, uint32_t _box_x=128, uint32_t _box_y=128, uint32_t _box_z=128);
 ...
 */
template < class IngredientsType >
UpdaterCreatorHyperstar<IngredientsType>::UpdaterCreatorHyperstar(IngredientsType& ingredients_, uint32_t _maxfunctionality, uint32_t _box_x, uint32_t _box_y, uint32_t _box_z):
 ...
 */
template < class IngredientsType >
void UpdaterCreatorHyperstar<IngredientsType>::initialize(){
 ...
 */
template < class IngredientsType >
bool UpdaterCreatorHyperstar<IngredientsType>::execute(){
	std::cout << "executeUpdaterCreatorHyperstar" << std::endl;
	...
	while ((queueLeaves.size() > 0))
        {
            ...
            for(uint32_t k = maxfunc-1; k > 0; k--)
            {
              ...
              std::cout << idx << " --> " << addIdx << " .f: " << k << std::endl;
              
              MaxNodeFunctionality[addIdx] = k;
...
template<class IngredientsType>
bool UpdaterCreatorHyperstar<IngredientsType>::addMonomerToParentWithProbability(uint32_t parent_id, int32_t type){
 ...
 */
template < class IngredientsType >
void UpdaterCreatorHyperstar<IngredientsType>::cleanup(){
    ...
    commentHG_FunctionalityNodes << " File produced by analyzer UpdaterCreatorHyperstar" << std::endl
